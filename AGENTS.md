...
- Use Codex's Agent tool for spawning agents, not just MCP
...
- MUST spawn concurrent agents using Codex's Agent tool
...
| `memory_import_claude` | Import Codex memories into AgentDB (allProjects=true for all) |
| `memory_search_unified` | Search across ALL namespaces (Codex + AgentDB + patterns) |
...
# Import all Codex memories into AgentDB
node .Codex/helpers/auto-memory-hook.mjs import-all
```

### Codex ↔ AgentDB Bridge

Codex auto-memory files (`~/.Codex/projects/*/memory/*.md`) are automatically imported into AgentDB with ONNX vector embeddings on session start. Use `memory_search_unified` to search across both stores.
...
| **Codex Bridge** | `memory_import_claude`, `memory_bridge_status` | Import Codex memories into AgentDB |
...
- **Codex Bridge**: Auto-imports MEMORY.md files into AgentDB on session start
- **Unified Search**: `memory_search_unified` searches Codex memories + AgentDB + patterns
...
- **Codex Agent tool** handles execution: agents, file ops, code generation, git
